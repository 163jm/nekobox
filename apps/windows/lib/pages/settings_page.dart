...
/// 设置页(桌面):本地服务 / 系统代理 / 代理模式 / 外观 / 测速 / 日志。
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _loadSysProxyState() async {
    final enabled = await WindowsSystemProxy.isEnabled();
    if (mounted) setState(() => _proxyEnabled = enabled);
  }
      ...
      body: ListView(
        children: [
          ...
          SwitchListTile(
            ...
            title: const Text('连接时自动设置系统代理'),
            value: settings.systemProxy,
            onChanged: (v) async {
              final s = settings.copy();
              s.systemProxy = v;
              await Repository.instance.updateSettings(s);
              if (!v) await WindowsSystemProxy.disable();
            },
          ),
          ListTile(
            leading: Icon(
              _proxyEnabled ? Icons.toggle_on : Icons.toggle_off,
              color: _proxyEnabled ? scheme.primary : scheme.outline,
            ),
            title: const Text('系统代理状态'),
            subtitle: Text(_proxyEnabled
                ? '已启用 (${settings.localPort} 端口)'
                : '未启用'),
            onTap: () async {
              if (_proxyEnabled) {
                await WindowsSystemProxy.disable();
              } else {
                await WindowsSystemProxy
                    .enable('127.0.0.1:${settings.localPort}');
              }
              await _loadSysProxyState();
              _toast(_proxyEnabled ? '系统代理已启用' : '系统代理已关闭');
            },
          ),
          const Divider(),
          _section('代理模式'),
          ...
          ListTile(
            ...
            subtitle: Text(settings.clashApiEnabled
                ? CoreEnv.dashboardUrl(settings.clashApiPort)
                : '启用 Clash API 后可用'),
            enabled: settings.clashApiEnabled,
            trailing: const Icon(Icons.chevron_right),
            onTap: () => _openDashboard(settings.clashApiPort),
          ),
          ListTile(
          ...
          SwitchListTile(
            ...
            title: const Text('IPv6 支持'),
            value: settings.ipv6Mode != 0,
            onChanged: (v) {
              final s = Repository.instance.settings.copy();
              s.ipv6Mode = v ? 1 : 0;
              Repository.instance.updateSettings(s);
          ...
          ListTile(
            ...
            subtitle: const Text('保存当前全部数据到备份目录'),
            trailing: const Icon(Icons.chevron_right),
            onTap: _doBackup,
          ),
          ListTile(
            ...
            child: Row(
              children: [
                Radio<String>(
                    ...
                    groupValue: current,
                    onChanged: null),
                const Text('TUN 全局(需 wintun.dll + 管理员权限)'),
              ],
            ),
          ),
        ],
