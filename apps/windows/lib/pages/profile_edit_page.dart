...
class _ProfileEditPageState extends State<ProfileEditPage> {
  ...
  void _initControllers() {
    for (final f in _fieldsFor(_draft.type)) {
      final value = _draft.bean[f.key] ?? f.defaultValue;
...
List<ProfileField> _fieldsFor(String type) {
  final common = const [
    ProfileField.text('name', '名称', hint: '节点名称', required: false),
    ProfileField.text('serverAddress', '服务器地址', hint: 'IP 或域名'),
    ProfileField.int('serverPort', '端口'),
  ];
  ...
  switch (type) {
    ...
    case ProfileType.naive:
      // NaïveProxy(sing-box naive outbound)
      return [
        ...common,
    ...
    case ProfileType.mieru:
      // Mieru:sing-box 不支持,仅保存配置
      return [
        ...common,
        ProfileField.text('username', '用户名', required: false),
        ProfileField.text('password', '密码', isPassword: true, required: false),
        ProfileField.text('cipher', 'Cipher', required: false),
        ProfileField.text('hash', 'Hash', required: false),
      ];
